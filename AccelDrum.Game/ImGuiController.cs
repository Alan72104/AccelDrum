...
namespace AccelDrum
{
    public class ImGuiController : IDisposable
    {
        ...
        private CursorState _lastWindowCursorState;
...
void main()
{
    outputColor = color * texture(in_fontTexture, texCoord);
        ...
        private void UpdateImGuiInput(GameWindow window)
        {
            ...
            // Skip 1 frame to prevent receiving a random coord after ungrabbing
            if (_lastWindowCursorState == window.CursorState && window.CursorState == CursorState.Normal)
            {
            ...
            _lastWindowCursorState = window.CursorState;

            var cursor = ImGui.GetMouseCursor();
            if (cursor != _lastCursor)
            {
                _lastCursor = cursor;
                window.Cursor = cursor switch
                {
                    ...
                    ImGuiMouseCursor.ResizeAll => MouseCursor.Default,
                    ImGuiMouseCursor.ResizeNS => MouseCursor.VResize,
                    ImGuiMouseCursor.ResizeEW => MouseCursor.HResize,
                    ImGuiMouseCursor.ResizeNESW => MouseCursor.Default,
                    ImGuiMouseCursor.ResizeNWSE => MouseCursor.Default,
                    ImGuiMouseCursor.Hand => MouseCursor.Hand,
                    ImGuiMouseCursor.NotAllowed => MouseCursor.Default,
